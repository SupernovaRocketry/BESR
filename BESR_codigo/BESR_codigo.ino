...
unsigned long   millisAtual, millisAnterior;
unsigned long   millisTela;
unsigned long   millisExpira;
unsigned long   millisIniciou;
unsigned long   millisDado;
...
LiquidCrystal_I2C tela(0x27, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE);
...
void inicializa() {
  ...
  if (!SD.begin(PINO_SD_CS)) {
    ...
    arquivoLog = SD.open(nomeConcat, O_CREAT | O_WRITE);
  }
  ...
  float zero = celulaCarga.get_units(50);
...
void espera() {
  ...
  if (debounceBotao()) {
    ...
    telaMudou = 1;
    novoarquivo = 0;
    millisIniciou = millis();
    millis1s = millisIniciou;
  }
...
void medir() {
  ...
  millisDado = millis() - millisIniciou;
...
//
//
//    while (parar==false && novoarquivo==0)
//    {
//      sprintf(nomeConcat, "BESR%d.txt", n);
//      if (SD.exists(nomeConcat))
//        n++;
